...
def animateJointCoordinates(joint_coordinates, connexion_tuples):
    ...
    for t in range(joint_coordinates.shape[2]):
        ...
        ax.scatter(
            [x_spine_mid + axis_length, x_spine_mid, x_spine_mid],
            [y_spine_mid, y_spine_mid + axis_length, y_spine_mid],
            [z_spine_mid, z_spine_mid, z_spine_mid + axis_length],
            marker="v",
            color="green",
        )

        x_axis = plt3d.art3d.Line3D(
            [x_spine_mid, x_spine_mid + axis_length],
            [y_spine_mid, y_spine_mid],
            [z_spine_mid, z_spine_mid],
        )
        x_axis.set_color("green")
        y_axis = plt3d.art3d.Line3D(
            [x_spine_mid, x_spine_mid],
            [y_spine_mid, y_spine_mid + axis_length],
            [z_spine_mid, z_spine_mid],
        )
        y_axis.set_color("green")
        z_axis = plt3d.art3d.Line3D(
            [x_spine_mid, x_spine_mid],
            [y_spine_mid, y_spine_mid],
            [z_spine_mid, z_spine_mid + axis_length],
        )
        ...
        # Subject coordinates
        ax.set_xlim3d(
            min(np.amin(joint_coordinates[:, x, :]), -axis_length),
            max(np.amax(joint_coordinates[:, x, :]), axis_length),
        )
        ax.set_ylim3d(
            min(np.amin(joint_coordinates[:, y, :]), -axis_length),
            max(np.amax(joint_coordinates[:, y, :]), axis_length),
        )
        ax.set_zlim3d(
            min(np.amin(joint_coordinates[:, z, :]), -axis_length),
            max(np.amax(joint_coordinates[:, z, :]), axis_length),
        )

        ax.scatter(
            joint_coordinates[:, x, t],
            joint_coordinates[:, y, t],
            joint_coordinates[:, z, t],
            color="blue",
        )
        ...
        for i in range(connexion_tuples.shape[0]):
            ...
            joint_line = plt3d.art3d.Line3D(
                [joint_coordinates[j1, x, t], joint_coordinates[j2, x, t]],
